# HausaNovels Android TWA v2.1.7
...
- Version: `2.1.7` (`versionCode 217`)

## Important GitHub update method

Upload this as a clean replacement. Do not only add the new files over an old project without deleting obsolete files. Version 2.1.7 is a clean source tree. The workflow no longer deletes project files during the build. It validates the source, builds from a clean Gradle state, and reads certificate fingerprints from the APKs that were actually produced.
...
`Actions → Build HausaNovels TWA v2.1.7 → Run workflow`
...
## v2.1.7 runtime repair

This package keeps `ng.hausanovels.app` and the original v2.1.7 version identifiers. Replace the repository contents with this clean source while preserving the repository's GitHub Actions secrets. The launcher now has a safe external-browser fallback and cannot route that fallback back into the HausaNovels app.

## Legacy-file-safe builds

This v2.1.7 package automatically removes obsolete files left by older GitHub uploads
