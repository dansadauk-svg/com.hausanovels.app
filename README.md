# HausaNovels Android TWA v2.1.8
...
- Version: `2.1.8` (`versionCode 218`)

## Important GitHub update method

Upload this as a clean replacement. Do not only add the new files over an old project without deleting obsolete files. Version 2.1.8 is a clean source tree. The workflow no longer deletes project files during the build. It validates the source, builds from a clean Gradle state, and reads certificate fingerprints from the APKs that were actually produced.
...
`Actions → Build HausaNovels TWA v2.1.8 → Run workflow`
...
## v2.1.8 runtime repair

This package keeps `ng.hausanovels.app` and updates the release identifiers to v2.1.8. Replace the repository contents with this clean source while preserving the repository's GitHub Actions secrets. The launcher now has a safe external-browser fallback and cannot route that fallback back into the HausaNovels app.

## Legacy-file-safe builds

This v2.1.8 package automatically removes obsolete files left by older GitHub uploads
